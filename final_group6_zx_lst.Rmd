...
$$W_2 \gets f_W(U_{W_2})$$
$$W_3 \gets f_W(U_{W_3})$$
$$W_4 \gets f_W(W_1, W_2, W_3, U_{W_4})$$
$$A \gets f_W(U_A, W_3, W_4)$$
$$Y \gets f_W(U_Y, A, W_3, W_4)$$
...
We use glm() function estimate E(Y|A,W) based on the following parametric regression model:
$E(Y|A,W)=expit(β_0+β_1W_3+β_2W_4)$

The average treatment effect (ATE) by parametric G-computation:
```{r}
...
We also use super learner to estimate E(Y|A,W), and get simple substitution estimator based on the G-Computation formula.

The statistical parameter by substituting the targeted predictions into the G-Computation formula:
```{r}
...
The $\hat \theta$ from IPTW means that the counterfactual probability of survival of patient would be 0.0168 units higher if all patients receive Radiation than if all patients receive surgery. 
...
IPTW gives the largest number, and the estimate of simple substitution gives the smallest number.
...
hist(result[,1], main="G-computation", xlab = "Point estimates")
hist(result[,2], main="IPTW", xlab = "Point estimates")
hist(result[,3], main="TMLE", xlab = "Point estimates")
hist(result[,4], main="AIPW", xlab = "Point estimates")
```

In the plot above, the number of loop for bootstrap is 40 so that we get 40 estimation of $\hat θ$ for each method we use above and we observe the distribution for each bootstrap method. From the plot, we observe that the distribution obtained from G-computation, IPTW and AIPW estimator is right-skewed. The distribution obtained from TMLE appears to be symmetric. From the interval that contains the most values, we observe that the intervals for all methods lie on -0.02 ~ 0.05.


#### Reference
1. Chen JJ, Stessin A, Christos P, Wernicke AG, Nori D, Parashar B. Differences in survival outcome between stage I and stage II glottic cancer: A SEER-based analysis. Laryngoscope. 2015;125(9):2093-2098. doi:10.1002/lary.25338
